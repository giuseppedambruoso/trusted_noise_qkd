...
ax1.plot(deltar_df['alpha'], deltar_df['deltar'], 
...
contour_filled = ax2.tricontourf(df['alpha'], df['p'], df[q_col], 
                                 levels=20, cmap='viridis', alpha=0.9)

# 2. Line contours overlaid (adds crisp boundaries to the color shifts)
contour_lines = ax2.tricontour(df['alpha'], df['p'], df[q_col], 
                               levels=20, colors='black', linewidths=0.4, alpha=0.6)
